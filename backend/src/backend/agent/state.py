...
class AgentState(TypedDict):
    ...
    scrape_history: NotRequired[Annotated[list[dict[str, Any]], operator.add]] #History of scraped content during conversation; entries tagged with query_index — response node only receives the current query's entries
    tool_insights: NotRequired[Annotated[list[dict[str, Any]], operator.add]] #Per-tool interpretations written by insight_node ({tool_name, group, insight, cycle, query_index}); response node only receives the current query's entries; streamed to frontend
    ...
    dialogue: NotRequired[Annotated[list[AnyMessage], operator.add]] #Ordered human turns and final AI responses only; no tool messages
